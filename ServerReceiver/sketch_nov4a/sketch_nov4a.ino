...
LiquidCrystal_I2C lcd(0x27, 16,2); //oggetto lcd
...
void stampa_non_disp_schermo(LiquidCrystal_I2C lcd){
  lcd.setCursor(0,0);
  lcd.print("Posti non disponibili!");
}
    ...
    if(posti_liberi > 0){  //Se ci sono posti liberi...
      ...
      if(message->x == 0 && posti_liberi < posti_totale ){
        ...
        Serial.printf("%02x:%02x:%02x:%02x:%02x:%02x\n",
         ...
         esp_err_t result = esp_now_send(macMatrics[message->id-1], (uint8_t *) &myAck, sizeof(myAck));
         posti_liberi = increasePlaces(result, posti_liberi); //incremento posti nel parcheggio
         stampa_posti_schermo(lcd,posti_liberi);
