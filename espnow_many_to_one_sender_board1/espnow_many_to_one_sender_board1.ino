...
typedef struct struct_message {
    ...
    int x; //contenuto del messaggio da inviare (in questo caso, "una macchina deve entrare")
    uint8_t mac_address[6]; //mac address del mittente
} struct_message;
...
void OnAckRecv(const uint8_t * mac, const uint8_t *myAck_incoming, int len) {
  ...
  if(myAck.value==1){ //se il server mi manda ACK=1, allora posso far entrare la macchina
...
void loop() {
  ...
  myData.x = 1; //questa scheda invierà sempre 1 perché gestirà solo gli ingressi al parcheggio
